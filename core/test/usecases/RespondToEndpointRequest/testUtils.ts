...
// Parses the exact script shape generated by getConfigurationScript. This test
// helper is only for trusted, locally generated response fixtures; it is not a
// general JavaScript parser and deliberately never executes script content.
export function extractAppConfig(body: string) {
    const scriptPrefix = "window.APP_CONFIG=";
    const $ = load(body);
    const scriptContent = $("script#app-config").html();
    if (
        scriptContent === null ||
        !scriptContent.startsWith(scriptPrefix) ||
        !scriptContent.endsWith(";")
    ) {
        throw new Error("Unexpected app configuration script shape");
    }
    return JSON.parse(scriptContent.slice(scriptPrefix.length, -1));
}
