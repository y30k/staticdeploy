...
| **B-DNS**     | Trusted portal and wildcard published/preview DNS/TLS, registrable-site/browser matrix, trusted proxy chain, pre-app query-log redaction, cache bypass, fresh never-used content origins and permanent retirement after any worker allowance, internal exposure decision, route cutover access | External content acceptance and M7                |
