...
| **B-DNS**     | Trusted portal and wildcard published/preview DNS/TLS, trusted proxy chain, internal exposure decision, route cutover access                  | External content acceptance and M7                |
...
| **B-S3**      | S3-compatible service, create-only/conditional capability pass, policies, encryption/versioning/retention/backups, provider-chain credentials | M3/M6/M7 production acceptance                    |
...
| **B-DEPLOY**  | Kubernetes and/or Docker target, ingress, identities, policies/volumes, environment approval, rollback operator                               | M6/M7 production acceptance                       |
...
- **Follow-on:** canonical `main` migration and future release settings remain
  B-GH work owned by M2-01; they do not reopen M1-R1.
...
| **M2-02**   | **Pin deterministic runtime and workspace install.** Repository, CI, and builder pins agree; frozen clean installs do not change lockfile; remove Node 14 references.                                                                               | G-M2 pin/install phase twice; lockfile diff; retained workspace compile/test.                          | M2-01               | —                                                       |
...
| **M3-06**   | **Implement server-side OIDC sessions.** Code+PKCE/state/nonce validation, opaque host-only cookie, CSRF, logout/revocation/expiry/rotation; no v2 browser token storage.                                                                                      | AUTH-01 through AUTH-05 against mock IdP.                                                     | M3-03                      | B-OKTA for AUTH-06/production-like exit                                  |
| **M3-07**   | **Implement role bindings and authorization matrix.** Admin/owner/publisher/viewer/denied rules cover every write; group binding replacement is transactional/idempotent; no escalation or partial update; safe audits.                                        | API-03 and binding rollback/group-change cases.                                               | M3-02, M3-03, M3-06        | B-OKTA for real group mapping                                            |
| **M3-08**   | **Implement publication outbox and routing projection.** Atomic desired pointer/audit/outbox; generation file and conditional `current.json`; read-back acknowledgement; stale/duplicate/out-of-order safety; reconciler and last-known-good behavior.         | PROJ-01 through PROJ-06.                                                                      | M3-03, M3-04, M3-05        | B-S3 for production conditional-write/read semantics                     |
| **M3-09**   | **Split the four runtime commands.** Distinct config/startup/health and dependency composition; content has no DB/OIDC/quarantine/write configuration; wrong host fails closed.                                                                                | Command boot/config-negative tests, credential inspection, wrong-host/cross-origin tests.     | M3-05, M3-06, M3-08, M2-07 | —                                                                        |
| **M3-10**   | **Create Compose security foundation.** Run migrate/control/content/worker/PostgreSQL/MinIO with persistent volumes, distinct credentials, health, non-root/read-only roots, dropped capabilities and repeatable teardown.                                     | Compose config; start/health/restart/teardown; identity/mount/privilege inspection.           | M3-09                      | —                                                                        |
| **M3-11**   | **Create initial Helm security foundation.** Express the same commands/config/migration with distinct service accounts, configurable secure contexts/probes/resources/policies; provider annotations optional.                                                 | Chart lint/schema/template and disposable-cluster install/network/identity/migration restart. | M3-10                      | B-DEPLOY only for production target acceptance                           |
...
| **M4-09**   | **Serve isolated preview and published content.** Host-derived routing only; short-lived preview cookie/capability; selected default and assets served from immutable release; cross-app/portal state denied.          | R5/R11 content-security and PROJ-06 tests.                                          | M4-06, M4-08, M3-09        | B-DNS for production origin/TLS acceptance |
| **M4-10**   | **Modernize frontend build/test foundation.** Replace CRA with Vite and Enzyme with Testing Library; upgrade React/Ant Design compatibly; retain only required UI behavior/CSP compatibility.                          | Frontend build/component/serve smoke; dependency search.                            | M3-GATE                    | —                                          |
| **M4-11**   | **Modernize routing/forms.** Supported router and maintained form library replace React Router 5/Redux Form without a visual redesign; retained routes/validation/deep links work.                                     | Route, deep-link, history, validation and submission-error component/browser tests. | M4-10                      | —                                          |
...
| ID        | Story and unique acceptance                                                                                                                                                                                                                                                                                                      | Validation                                                                     | Depends on                 | External blockers                   |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | -------------------------- | ----------------------------------- |
| **M6-01** | **Freeze profile/upgrade contract.** Document shared image/config/migration/storage/health/smoke names, ingress/secret/provider-chain behavior, supported upgrade/schema rollback boundary and operator ownership; no cloud/CD controller requirement.                                                                           | Cross-profile design review against disposable Compose/cluster.                | M5-GATE                    | B-DEPLOY for production values only |
...
| **M6-07** | **Add explicit rollback workflows.** Select prior verified digest, reject incompatible schema, run identical smoke, retain actor/target/from/to/result evidence.                                                                                                                                                                 | DEP-04 successful and intentionally incompatible cases for both profiles.      | M6-06                      | B-GH, B-DEPLOY                      |
...
```text
M1-R1
 -> M2-01 -> M2-02 -> M2-03/04/05 -> M2-06 -> M2-07 -> M2-08 -> M2-GATE
 -> M3-01 -> M3-02/03/04/05/06/07/08 -> M3-09 -> M3-10 -> M3-11 -> M3-GATE
 -> M4-01..09 + M4-10/11 -> M4-12 -> M4-GATE
 -> M5-01..07 -> M5-GATE
