...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `control` | Portal, API v2, OIDC callbacks, server sessions, authorization, metadata, direct-upload streaming, audit, idempotency, publication requests | Application/session PostgreSQL role; quarantine write scoped to upload sessions; routing status read; no release write |
| `content` | Resolve published/preview host, validate projection/capability, serve immutable content with sealed headers, cache last-known-good routing  | Read-only routing/release storage; preview verification public key; no PostgreSQL, OIDC, quarantine, or write access   |
| `worker`  | Claim jobs, validate/scan/hash, build deterministic downloads/manifests, finalize releases, apply publication outbox, clean quarantine      | Worker PostgreSQL role; quarantine read/delete; release and routing writes                                             |
...
| DNS/TLS/exposure | Portal, wildcard published/preview hosts, certificates, trusted proxy chain, internal viewer policy                                         | External preview/publishing and M7                |
| PostgreSQL       | Production service, roles, migration identity, backups, restore owner, RPO/RTO                                                              | M3/M6/M7 production acceptance                    |
| Object storage   | Endpoint/bucket/prefix, capability pass, policies, encryption/versioning/retention/backups, credentials/provider chain                      | M3/M6/M7 production acceptance                    |
| Content security | Quotas/retention, ZIP limits, malware engine/failure posture, exception authority                                                           | M5 production acceptance                          |
| Deployment       | Kubernetes and/or Docker host, ingress, identities, network policy/PV support, approvals, rollback operator                                 | M6/M7 production acceptance                       |
