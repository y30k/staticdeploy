...
2. A preview capability is a compact asymmetric JWS with a purpose-specific
   ...
   `session_exp`; URL validation also requires `exchange_exp`. Embedded preview
   is allowed only from the exact portal by CSP and sandbox; top-level preview
   is the compatibility fallback when approved browsers block third-party
   cookies. B-DNS must validate registrable-site/browser behavior.
...
| ORG-03   | High     | Persistent service worker controls a published origin across release transitions     | ISO-01 denies workers; no transition silently retains an unapproved worker                 | ADR-006; M4-09; TM-ISO-01                   |
...
| B-DNS  | Portal and published/preview wildcard DNS/TLS, registrable-site/browser matrix, trusted proxy chain, pre-app query-log redaction, cache bypass, exposure/viewer policy, route-cutover access                              | Host/origin/capability logic on reserved local domains | Real DNS/TLS/proxy/cache/log/wildcard isolation is accepted                 |
